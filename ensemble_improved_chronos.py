#!/usr/bin/env python
"""
Ensemble: Improved Powell (17.46) + Chronos (Deep Learning)
Goal: 17.5+ score
"""
...
print("="*80)
print("Ensemble: Improved Powell (17.46) + Chronos (Best DL)")
print("="*80)
...
if not os.path.exists('best_chronos.pth'):
    ...
    # Generate Chronos predictions for 8790~8989
    SEQ_LEN = 60
    chronos_preds = []
...
if chronos_available:
    ...
    for w_powell in np.arange(0.5, 1.05, 0.05):
        w_chronos = 1 - w_powell

        ensemble_preds = w_powell * powell_preds + w_chronos * chronos_preds
        ...
        try:
            ...
            if ens_score > best_ensemble_score:
                ...
                print(f"  NEW BEST: w_powell={w_powell:.2f}, w_chronos={w_chronos:.2f} -> Score: {ens_score:.4f}")
        except Exception as e:
            pass

    print(f"\nBest Ensemble: w_powell={best_weight:.2f}, w_chronos={1-best_weight:.2f}")
...
if chronos_available:
    ...
    print(f"Improvement:  {best_ensemble_score - powell_score:+.4f}")
else:
...
submission_df.to_csv("submission_ensemble_improved_chronos.csv", index=False)
print(f"\nSubmission saved to submission_ensemble_improved_chronos.csv")
